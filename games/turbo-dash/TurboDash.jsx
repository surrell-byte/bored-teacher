...
function PlayerCard({ player, index, active }) {
  return (
    <aside className={`td-sidebar td-player-${index + 1} ${active ? 'active' : ''}`}>
      <div className="td-avatar-disc">{player.avatar}</div>
      <strong>{player.name}</strong>
      <div className="td-progress"><span style={{ height: `${(player.position / FINISH) * 100}%` }} /></div>
