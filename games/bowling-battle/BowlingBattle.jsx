    ...
    <main className={`bowling-battle-game is-${screen}${screen === "game" ? " is-wide" : ""}`}>
      ...
      {screen === "game" && game && <section className="bb-screen active bb-game-screen">
        ...
        <aside className="bb-side-column">
          ...
          <div className={`bb-turn-banner ${game.currentPlayerIdx === 0 ? "bb-p1" : "bb-p2"}`}><span className="bb-turn-dot" aria-hidden="true" /><div><small>PLAYER {game.currentPlayerIdx + 1}</small><strong>{currentPlayer?.name}</strong><b>YOUR TURN</b></div></div>
          <div className="bb-roll-indicators" aria-label={`Frame ${currentPlayer?.frame}, roll ${currentPlayer?.rollInFrame}`}><span className={currentPlayer?.rollInFrame === 1 ? "active" : ""}>ROLL 1</span><span className={currentPlayer?.rollInFrame === 2 ? "active" : ""}>ROLL 2</span>{currentPlayer?.frame === 10 && <span className={currentPlayer?.rollInFrame === 3 ? "active" : ""}>ROLL 3</span>}</div>
          ...
          {game.mode === "card" && <div className="bb-card-move"><div className="bb-card-heading"><div><strong>CHOOSE YOUR ROLL</strong><span>Pick a card to reveal your pin challenge.</span></div>{game.chosenCard && <b className="bb-card-result">{game.chosenCard.knocked} PINS</b>}</div><div className="bb-card-grid">{game.cards.map((card) => <button type="button" key={card.id} className={`bb-pick-card${game.chosenCard?.id === card.id ? " flipped" : ""}`} onClick={() => chooseCard(card)} disabled={game.phase !== "cardPick"} aria-label={game.chosenCard?.id === card.id ? `Card ${card.id}, ${card.knocked} pins` : `Choose hidden card ${card.id}`}><span className="bb-card-number">{card.id}</span>{game.chosenCard?.id === card.id ? <><strong>{card.knocked}</strong><small>{card.knocked === 1 ? "PIN" : "PINS"}</small></> : <span className="bb-card-back-mark">?</span>}</button>)}</div></div>}
        </aside>
      </section>}
