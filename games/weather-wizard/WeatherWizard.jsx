...
export default function WeatherWizard({ onComplete, onHudUpdate }) {
  ...
  if (screen === 'levels') return <main className="weather-wizard ww"><section className="ww-panel ww-adventure-panel"><span className="ww-badge">WIZARD ACADEMY</span><h1>Your Adventure</h1><div className="ww-levels">{LEVELS.map((entry, index) => <button key={entry.title} disabled={index > 0 && !completed.includes(index - 1)} onClick={() => startLevel(index)}>{index > 0 && !completed.includes(index - 1) ? '🔒' : entry.icon}<strong>{index + 1}. {entry.title}</strong><small>{completed.includes(index) ? '✓ Completed' : entry.description || 'Start level'}</small></button>)}</div></section></main>;
