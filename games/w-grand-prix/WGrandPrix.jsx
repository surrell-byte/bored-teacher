...
function buildSpecials() {
  ...
  [3,6,10,13,16,19].forEach((pos,i) => {
    const effects=[
      {delta:-1,label:"🛢️ Oil Slick! -1"},
      {delta:+2,label:"💨 Slipstream! +2"},
      {delta:-2,label:"💥 Crash! -2"},
      {delta:+1,label:"🚦 Safety car ends +1"},
      {delta:+2,label:"⚡ DRS Zone! +2"},
      {delta:-1,label:"🪨 Debris! -1"},
    ];
    s[pos]=effects[i];
  });
  return s;
...
export default function WGrandPrix() {
  ...
  }, [animating, winner, current, names, numPlayers]);

  if (screen === "setup") return (
    <div style={{
      minHeight:"100vh", display:"flex", flexDirection:"column",
      alignItems:"center", justifyContent:"center",
      background:"linear-gradient(135deg,#0f0f0f,#1a0f00,#0f0000)",
      ...
      <div style={{ marginBottom:20 }}>
        <label style={{ color:"#f97316", fontWeight:700, marginRight:12 }}>Drivers:</label>
        {[2,3,4].map(n => (
          <button key={n} onClick={() => setNumPlayers(n)} style={{
            padding:"8px 18px", borderRadius:10, border:"none", marginRight:8,
            background: numPlayers===n ? "#f97316" : "#374151",
            color:"#fff", fontWeight:700, cursor:"pointer",
          }}>{n}</button>
        ))}
      </div>
      {Array.from({length:numPlayers},(_,i) => (
        <div key={i} style={{ display:"flex", gap:10, marginBottom:10, alignItems:"center", justifyContent:"center" }}>
          <span style={{ fontSize:"1.4rem" }}>{EMOJIS[i]}</span>
          <input value={names[i]} onChange={e => setNames(n => n.map((v,j)=>j===i?e.target.value:v))}
            style={{
  ...
  // Build track visualization
  const trackCells = Array.from({length:TRACK_TOTAL+1},(_,i)=>i);

  return (
    <div style={{
      minHeight:"100vh", display:"flex", flexDirection:"column",
      alignItems:"center",
      background:"linear-gradient(135deg,#0f0f0f,#1a0f00)",
      ...
      {/* Track */}
      <div style={{
        display:"flex", flexWrap:"wrap", gap:3, maxWidth:440, justifyContent:"flex-start",
        background:"rgba(255,255,255,0.03)", borderRadius:12, padding:10,
        marginBottom:14, border:"1px solid rgba(255,255,255,0.07)",
      }}>
        {trackCells.map(i => {
          const playersHere = positions.reduce((acc,pos,pi)=>{ if(pos===i) acc.push(pi); return acc; },[]);
          const isSpecial = !!SPECIALS[i];
          const isFinish = i === TRACK_TOTAL;
          return (
            <div key={i} style={{
              width:36, height:36, borderRadius:6, display:"flex", alignItems:"center",
              justifyContent:"center", fontSize:"0.65rem",
              background: isFinish?"#92400e": isSpecial?"rgba(239,68,68,0.2)":"rgba(255,255,255,0.04)",
              border: isSpecial?"1px solid #ef444466":"1px solid rgba(255,255,255,0.06)",
              position:"relative", flexDirection:"column",
            }}>
              {playersHere.length>0 ? (
                playersHere.map(pi=><span key={pi} style={{fontSize:"1rem",lineHeight:1}}>{EMOJIS[pi]}</span>)
              ) : (
                <span style={{color:"rgba(255,255,255,0.25)"}}>
                  {isFinish?"🏆":isSpecial?"⚡":i}
                </span>
              )}
            </div>
          );
      ...
      <div style={{ display:"flex", gap:10, marginBottom:12, flexWrap:"wrap", justifyContent:"center" }}>
        {Array.from({length:numPlayers},(_,i)=>(
          <div key={i} style={{
            ...
            background: i===current&&!winner?"rgba(249,115,22,0.15)":"rgba(255,255,255,0.04)",
            border:`1px solid ${i===current&&!winner?"#f97316":COLORS[i]}`,
            fontSize:"0.85rem",
          }}>
            {EMOJIS[i]} <span style={{color:COLORS[i],fontWeight:700}}>{names[i]}</span>
            <span style={{color:"#94a3b8",marginLeft:6}}>{positions[i]}/{TRACK_TOTAL}</span>
          </div>
        ))}
      </div>
      ...
      {winner !== null ? (
        <div style={{ textAlign:"center", marginBottom:16 }}>
          ...
          <div style={{ color:"#f97316", fontWeight:800, fontSize:"1.3rem", marginBottom:12 }}>
            {EMOJIS[winner]} {names[winner]} wins the Grand Prix!
          </div>
        ...
        <div style={{ textAlign:"center", marginBottom:12, width:"100%", maxWidth:440 }}>
          <p style={{ color:"#f97316", fontWeight:700, marginBottom:6 }}>
            {EMOJIS[current]} {names[current]}'s turn
            <span style={{
              marginLeft:12, background:"rgba(249,115,22,0.2)", padding:"3px 12px",
              borderRadius:999, fontSize:"0.85rem", color: timeLeft<=3?"#f87171":"#f97316"
            }}>⏱ {timeLeft}s</span>
          </p>
          <p style={{ color:"#64748b", fontSize:"0.8rem", marginBottom:10 }}>Play a card to advance</p>
          <div style={{ display:"flex", gap:10, justifyContent:"center", flexWrap:"wrap" }}>
            {cards.map((card,i) => (
              <button key={i} onClick={() => playCard(card)} disabled={animating}
                style={{
                  padding:"14px 18px", borderRadius:16, border:"none",
                  background: animating?"#1e293b":`linear-gradient(135deg,${card.delta>=2?"#f97316":card.delta===1?"#22c55e":card.delta===0?"#64748b":"#ef4444"},${card.delta>=2?"#ea580c":card.delta===1?"#16a34a":card.delta===0?"#475569":"#dc2626"})`,
                  color:"#fff", fontWeight:800, fontSize:"1.1rem", cursor:animating?"default":"pointer",
                  minWidth:80, boxShadow:animating?"none":"0 4px 12px #0004",
                }}>
                <div style={{ fontSize:"1.4rem" }}>{card.label}</div>
                <div style={{ fontSize:"0.7rem", opacity:0.8 }}>{card.desc}</div>
              </button>
            ))}
          </div>
        </div>
