...
export default function WordFusion({ onComplete }) {
  ...
  const startLevel = useCallback((id) => {
    ...
    setIdx(0); setScore(0); setMistakes(0); setSelected(null); setAnswered(false); setScreen("game");
  }, []);
  ...
  const next = () => {
    if (idx + 1 >= questions.length) {
      const accuracy = Math.round((questions.length / (questions.length + mistakes)) * 100);
      if (levelId === "easy" && accuracy >= 70) setUnlocked(u => ({ ...u, medium:true }));
      if (levelId === "medium" && accuracy >= 70) setUnlocked(u => ({ ...u, hard:true }));
  ...
  if (screen === "landing") return (
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"linear-gradient(135deg,#e0f7fa,#b2ebf2,#e8f5e9)", fontFamily:"'Segoe UI',sans-serif", padding:24, textAlign:"center" }}>
      ...
      <div style={{ display:"flex", gap:12, flexWrap:"wrap", justifyContent:"center" }}>
        {LEVELS.map(lvl => (
          <button key={lvl.id} onClick={() => unlocked[lvl.id] && startLevel(lvl.id)} style={{ padding:"14px 28px", borderRadius:16, border:"none", background: unlocked[lvl.id] ? lvl.color : "#e5e7eb", color: unlocked[lvl.id] ? "#fff" : "#9ca3af", fontWeight:800, fontSize:"1rem", cursor: unlocked[lvl.id] ? "pointer" : "not-allowed", boxShadow: unlocked[lvl.id] ? `0 4px 16px ${lvl.color}66` : "none" }}>{unlocked[lvl.id] ? lvl.label : `🔒 ${lvl.label}`}</button>
        ))}
      </div>
    </div>
  );
  ...
  if (screen === "complete") return (
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"linear-gradient(135deg,#e0f7fa,#b2ebf2)", fontFamily:"'Segoe UI',sans-serif", padding:24, textAlign:"center" }}>
      <div style={{ fontSize:"4rem", marginBottom:12 }}>🏆</div>
      <h2 style={{ fontSize:"2rem", color:"#00695c", marginBottom:8 }}>{currentLevelDef.label} Complete!</h2>
      <p style={{ color:"#00796b", fontSize:"1.2rem", marginBottom:24 }}>Score: {score} / {questions.length * 10}</p>
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"linear-gradient(135deg,#e0f7fa,#b2ebf2,#e8f5e9)", fontFamily:"'Segoe UI',sans-serif", padding:24 }}>
      <button onClick={() => setScreen("landing")} style={{ position: "absolute", top: 20, left: 20, background: "none", border: "none", fontSize: "1.8rem", cursor: "pointer", color: "#00695c", padding: 8 }}>✕</button>
      <div style={{ width:"100%", maxWidth:520 }}>
        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:16 }}>
          <span style={{ background:currentLevelDef.color, padding:"4px 14px", borderRadius:999, color:"#fff", fontWeight:700 }}>{currentLevelDef.label}</span>
        ...
        <div style={{ display:"flex", justifyContent:"center", alignItems:"center", gap:20, marginBottom:24, background:"white", borderRadius:24, padding:"24px 20px", boxShadow:"0 8px 24px #00695c22" }}>
          ...
          <div style={{ fontSize:"1.8rem", color:"#94a3b8" }}>=</div>
          <div style={{ background:"#fef3c7", padding:"14px 24px", borderRadius:16, fontWeight:800, fontSize:"1.8rem", color:"#92400e", minWidth:120, textAlign:"center" }}>{answered ? current.answer : "❓"}</div>
        </div>
        <div style={{ display:"flex", flexWrap:"wrap", gap:10, justifyContent:"center", marginBottom:16 }}>
          {current.options.map(opt => {
            const isCorrect = answered && opt === current.answer;
            const isWrong = answered && opt === selected && opt !== current.answer;
            return (<button key={opt} onClick={() => answer(opt)} style={{ padding:"12px 24px", borderRadius:999, border:"none", cursor: answered?"default":"pointer", fontWeight:700, fontSize:"0.95rem", background: isCorrect?"#22c55e":isWrong?"#ef4444":"#00695c", color:"#fff", boxShadow: isCorrect?"0 0 12px #22c55e88":isWrong?"none":`0 4px 0 #004d40` }}>{opt}</button>);
          })}
        </div>
        {answered && (
          <div style={{ textAlign:"center" }}>
            <p style={{ fontWeight:700, color:selected===current.answer?"#16a34a":"#dc2626", marginBottom:12 }}>{selected===current.answer ? "🧩 Fused! Correct!" : `❌ Answer: "${current.answer}"`}</p>
            <button onClick={next} style={{ padding:"14px 32px", borderRadius:999, border:"none", background:"linear-gradient(135deg,#00695c,#00897b)", color:"#fff", fontWeight:800, cursor:"pointer" }}>{idx+1 < questions.length ? "➡️ Next" : "🏆 Finish"}</button>
          </div>
        )}
      </div>
    </div>
