...
export default function WordMatch() {
  ...
  const [matched, setMatched] = useState(new Set()); // matched by word
  ...
  useEffect(() => {
    ...
    if (selectedWord === selectedMeaning) {
      ...
      setFeedback("✅ Correct!");
      setScore(s => s + 1);
      if (next.size === DATA.length) { setDone(true); setFeedback("🎉 All matched!"); }
    } else {
  ...
  return (
    <div style={{
      ...
      fontFamily: "'Segoe UI', sans-serif", padding: 24, color: "#f1f5f9",
    }}>
      <h1 style={{ fontSize: "2rem", margin: "0 0 4px", color: "#818cf8" }}>🔤 Word Match</h1>
      <p style={{ color: "#94a3b8", margin: "0 0 4px" }}>
        Score: <strong style={{ color: "#f5c842" }}>{score}</strong> / {DATA.length}
      </p>
      <p style={{
        margin: "0 0 20px", fontWeight: 700, fontSize: "1rem",
        color: feedback.startsWith("✅") || feedback.startsWith("🎉") ? "#4ade80" : "#f87171",
        minHeight: 24,
      }}>{feedback}</p>
      ...
      <div style={{ display: "flex", gap: 24, alignItems: "flex-start", flexWrap: "wrap", justifyContent: "center" }}>
          ...
          {DATA.map(({ word }) => {
            ...
            return (
              <button key={word}
                ...
                style={{
                  padding: "12px 20px", borderRadius: 12, border: "none", cursor: isMatched ? "default" : "pointer",
                  fontWeight: 700, fontSize: "1rem",
                  background: isMatched ? "#166534" : isSelected ? "#4f46e5" : "#1e293b",
                  color: isMatched ? "#4ade80" : isSelected ? "#fff" : "#cbd5e1",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, maxWidth: 260 }}>
          ...
          {shuffledMeanings.map(({ word, meaning }) => {
            ...
            return (
              <button key={word}
                onClick={() => !isMatched && setSelectedMeaning(word)}
                style={{
                  padding: "12px 16px", borderRadius: 12, border: "none", cursor: isMatched ? "default" : "pointer",
      ...
      {done && (
        <button onClick={startGame} style={{
          marginTop: 28, padding: "14px 36px", borderRadius: 999, border: "none",
