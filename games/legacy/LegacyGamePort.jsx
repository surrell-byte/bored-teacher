...
export default function LegacyGamePort({ gameId, onComplete }) {
  ...
  return <main className="legacy-port" data-mode={config.mode}>
    ...
    <section className="legacy-port-card">
      <header className="legacy-port-header"><span>{config.icon}</span><div><p>React game port</p><h1>{config.title}</h1></div><div className="legacy-port-score">{score}<small>points</small></div></header>
