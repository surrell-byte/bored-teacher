...
function TargetSvg({ shakeKey }) {
  return <svg className={`ad-target ${shakeKey ? 'target-shake' : ''}`} viewBox="0 0 90 130" aria-label="archery target" role="img">
...
export default function ArchersDuel({ onComplete }) {
  ...
  const impact = (x, y, pts) => {
    ...
    const frame = (now) => {
      ...
      ctx.save(); ctx.globalAlpha = (1 - t) * .85; ctx.beginPath(); ctx.arc(x, y, t * 65, 0, Math.PI * 2); ctx.strokeStyle = color; ctx.lineWidth = 5; ctx.shadowBlur = 18; ctx.shadowColor = color; ctx.stroke(); ctx.restore();
  ...
  const animateArrow = (pts, turn, done) => {
    ...
    const startX = turn === 'student' ? 100 : width - 290; const startY = 150; const endX = pts > 0 ? width - 145 : width - 70; const endY = 120 + (pts > 0 ? Math.random() * 20 - 10 : -35);
  ...
  const chooseTile = (idx) => {
    ...
    setTimeout(() => animateArrow(shot.pts, currentTurn, () => {
      setScores((previous) => {
  ...
  return <div className="archer-duel-root">
    ...
    {screen === 'game' && <div id="gameScreen"><div className="scoreboard"><div className={`score-panel student${currentTurn === 'student' ? ' active-turn' : ''}`}><div className="role">Student</div><div className="player-name">{COLORS[picks.student].name} Archer</div><div className="pts-value">{scores.student}</div><div className="pts-label">points</div><div className="progress-track"><div className="progress-fill-s" style={{ width: `${Math.min(100, scores.student / WIN_TARGET * 100)}%` }} /></div></div><div className="score-divider">⚔</div><div className={`score-panel teacher${currentTurn === 'teacher' ? ' active-turn' : ''}`}><div className="role">Teacher</div><div className="player-name">{COLORS[picks.teacher].name} Archer</div><div className="pts-value">{scores.teacher}</div><div className="pts-label">points</div><div className="progress-track"><div className="progress-fill-t" style={{ width: `${Math.min(100, scores.teacher / WIN_TARGET * 100)}%` }} /></div></div></div><div className="arena-wrap" ref={arenaRef}><ArenaScene /><TargetSvg shakeKey={shakeKey} />{currentTurn === 'student' ? <ArcherSvg colorKey={picks.student} facing="right" width="95" height="145" viewBox="0 0 72 110" className="ad-student" /> : <ArcherSvg colorKey={picks.teacher} facing="left" width="95" height="145" viewBox="0 0 72 110" className="ad-teacher" />}<canvas ref={canvasRef} className="ad-canvas" /></div>{floatingPoints && <div key={floatingPoints.id} className={`floating-points ${floatingPoints.value >= 0 ? 'positive' : 'negative'}`} onAnimationEnd={() => setFloatingPoints(null)}>{floatingPoints.value > 0 ? '+' : ''}{floatingPoints.value}</div>}<div className="turn-banner"><div className="turn-name">{COLORS[picks[currentTurn]].name} Archer ({currentTurn === 'student' ? 'Student' : 'Teacher'})'s turn</div><div className="turn-instruction">Pick a numbered tile — what's hidden inside?</div></div><div className="tiles-section"><div className="tiles-label">— SELECT A TILE —</div><div className="tiles-row">{currentShots.map((shot, index) => { const isRevealed = revealedIdx === index; const isChosen = chosenIdx === index; return <button key={`${shot.label}-${index}`} className={`tile tile-${index + 1}${isRevealed ? ' revealed' : ''}${isChosen ? ' chosen' : ''}${busy && !isChosen ? ' disabled' : ''}`} disabled={busy} onClick={() => chooseTile(index)}><div className="tile-face"><div className="tile-num">{index + 1}</div><div className="tile-q">TAP TO REVEAL</div></div><div className="tile-back"><div className="rev-icon">{shot.icon}</div><div className="rev-label">{shot.label}</div><div className={`rev-pts ${shot.pts > 0 ? 'pos' : shot.pts < 0 ? 'neg' : 'zero'}`}>{shot.pts > 0 ? '+' : ''}{shot.pts} pts</div></div></button>; })}</div></div><div id="resultBox" className={resultCls}>{resultText}</div></div>}
