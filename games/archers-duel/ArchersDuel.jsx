  ...
  [
    {
      ...
      icon: "🎯",
      pts: 30,
      desc: "Dead centre — perfect form",
    },
    { label: "Inner ring", icon: "⭕", pts: 15, desc: "Solid hit on the red" },
    {
      label: "Wide miss",
      icon: "💨",
      pts: -10,
      desc: "Arrow sails past the boss",
    },
  ],
  [
    { label: "Gold!", icon: "✨", pts: 40, desc: "Strikes the gold centre" },
    { label: "Graze", icon: "〰", pts: 5, desc: "Clips the outer white" },
    {
      label: "Lost arrow",
      icon: "❌",
      pts: -15,
      desc: "Completely off target",
    },
  ],
  [
    { label: "Black ring", icon: "◉", pts: 20, desc: "Solid black ring hit" },
    {
      label: "Outer miss",
      icon: "🌀",
      pts: 0,
      desc: "Arrow hits the backing — no score",
    },
    { label: "Stumble", icon: "😬", pts: -10, desc: "Slipped on release" },
  ],
  [
    {
      label: "Double gold",
      icon: "🌟",
      pts: 50,
      desc: "Back-to-back golds — legendary!",
    },
    { label: "Blue ring", icon: "🔵", pts: 10, desc: "Hits the blue zone" },
    {
      label: "Foul shot",
      icon: "⚠️",
      pts: -20,
      desc: "Line fault — heavy penalty",
    },
  ],
  [
    {
      ...
      icon: "🤚",
      pts: 25,
      desc: "Calm, controlled release",
    },
    {
      label: "Wind deflect",
      icon: "🌬️",
      pts: 0,
      desc: "Gust pushes it wide — no score",
    },
    {
      label: "Dropped nock",
      icon: "💢",
      pts: -5,
      desc: "Arrow slips off the string",
    },
  ],
];
const SPECIAL_SHOT = {
  label: "Orange Special",
  icon: "🟠",
  pts: 0,
  desc: "Perfect accuracy, burst impact, and double your score",
  special: true,
};
...
function ArcherSvg({ colorKey, facing, ...props }) {
  ...
  const transform =
    facing === "left" ? "scale(-1,1) translate(-72,0)" : undefined;
  return (
    <svg {...props} aria-label={`${c.name} archer`} role="img">
      <g transform={transform}>
        <rect x="24" y="74" width="10" height="28" rx="5" fill="#2c2c2c" />
        <rect x="38" y="74" width="10" height="28" rx="5" fill="#2c2c2c" />
        <rect x="21" y="95" width="14" height="8" rx="3" fill="#1a1a1a" />
        <rect x="37" y="95" width="14" height="8" rx="3" fill="#1a1a1a" />
        <rect x="22" y="44" width="28" height="34" rx="7" fill={c.body} />
        <rect x="8" y="46" width="11" height="20" rx="5" fill={c.sleeve} />
        <rect x="53" y="46" width="11" height="20" rx="5" fill={c.sleeve} />
        <circle cx="13" cy="67" r="5" fill={c.skin} />
        <circle cx="58" cy="67" r="5" fill={c.skin} />
        <ellipse cx="36" cy="30" rx="14" ry="15" fill={c.skin} />
        <ellipse cx="36" cy="17" rx="14" ry="8" fill={c.hair} />
        <rect x="22" y="17" width="28" height="8" fill={c.hair} />
        <circle cx="31" cy="30" r="2" fill="#2c1a0e" />
        <circle cx="41" cy="30" r="2" fill="#2c1a0e" />
        <path
          d="M64 38 Q72 60 64 82"
          fill="none"
          stroke={c.bow}
          strokeWidth="3"
          strokeLinecap="round"
        ...
        <line
          ...
          strokeLinecap="round"
        />
        <polygon points="70,57 77,60 70,63" fill="#8b6914" />
      </g>
    </svg>
  );
}
    ...
    >
      <defs>
        ...
        <linearGradient id="ad-ground" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#7ab84a" />
          <stop offset="100%" stopColor="#4a8020" />
        </linearGradient>
      </defs>
      <rect width="1100" height="280" fill="url(#ad-sky)" />
      <g fill="white" opacity=".72">
        <ellipse cx="180" cy="42" rx="65" ry="20" />
        <ellipse cx="220" cy="35" rx="42" ry="15" />
        <ellipse cx="720" cy="48" rx="60" ry="18" />
        <ellipse cx="765" cy="40" rx="35" ry="13" />
      </g>
      <rect y="205" width="1100" height="75" fill="url(#ad-ground)" />
      <rect y="205" width="1100" height="5" fill="#5a9030" />
      <g fill="#3a6825">
        <polygon points="360,48 330,130 390,130" />
        <polygon points="360,25 322,105 398,105" />
        <polygon points="470,70 442,140 498,140" />
        <polygon points="820,54 790,135 850,135" />
      </g>
      <g fill="#4a7030">
        <rect x="355" y="110" width="10" height="100" />
        <rect x="465" y="120" width="8" height="90" />
        <rect x="815" y="110" width="9" height="100" />
      </g>
      <rect x="970" y="170" width="5" height="40" fill="#8b6914" />
      <rect x="955" y="170" width="35" height="18" rx="2" fill="#c8a84b" />
      <text
        x="972"
        y="182"
        textAnchor="middle"
...
function TargetSvg({ shakeKey, targetRef }) {
  return (
    <svg
      ref={targetRef}
      className={`ad-target ${shakeKey ? "target-shake" : ""}`}
      viewBox="0 0 90 130"
      aria-label="archery target"
      role="img"
    >
      <rect x="38" y="80" width="6" height="50" fill="#7a5c18" rx="2" />
      <rect x="25" y="120" width="32" height="7" rx="3" fill="#5a3e10" />
      <ellipse cx="44" cy="46" rx="38" ry="42" fill="#c8a030" />
      <circle
        cx="44"
        cy="46"
        r="34"
        ...
        strokeWidth=".5"
      />
      <circle cx="44" cy="46" r="28" fill="#111" />
      <circle cx="44" cy="46" r="22" fill="#1a1aaa" />
      <circle cx="44" cy="46" r="16" fill="#cc2222" />
      <circle cx="44" cy="46" r="10" fill="#ffcc00" />
      <circle cx="44" cy="46" r="5" fill="#ffee88" />
    </svg>
  );
}
...
export default function ArchersDuel({ onComplete }) {
  ...
  const [specials, setSpecials] = useState({
    student: { unlocked: false, cooldown: 0 },
    teacher: { unlocked: false, cooldown: 0 },
  });
  ...
  const [resultText, setResultText] = useState(
    "The range is ready. Choose a tile to reveal your fate.",
  );
  ...
  const newRound = (turn) => {
    ...
    const shots = shuffle(SHOT_POOLS.flat()).slice(0, 3);
    if (specialsRef.current[turn].unlocked && specialsRef.current[turn].cooldown === 0) {
      shots.push(SPECIAL_SHOT);
    }
  ...
  const confirmPick = () => {
    ...
    if (setupStep === 0) {
      setPicks({ student: selectedColor });
      setSetupStep(1);
      setSelectedColor(null);
    } else startGame({ ...picks, teacher: selectedColor });
  };

  const drawArrow = (ctx, x, y, angle, pts, special = false) => {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(angle);
    ctx.beginPath();
    ctx.moveTo(-22, 0);
    ctx.lineTo(10, 0);
    ctx.strokeStyle = special ? "#ff8a18" : "#8b6914";
    ctx.lineWidth = special ? 5 : 3;
    ctx.lineCap = "round";
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(10, 0);
    ...
    ctx.lineTo(18, 3);
    ctx.closePath();
    ctx.fillStyle = special ? "#ffd166" : "#c0a030";
    ctx.fill();
    ctx.beginPath();
    ctx.moveTo(-22, 0);
    ctx.lineTo(-16, -5);
    ...
    ctx.lineTo(-16, 5);
    ctx.closePath();
    ctx.fillStyle = special ? "#ff8a18" : pts >= 0 ? "#c0392b" : "#555";
    ctx.fill();
    ctx.restore();
  };

  const impact = (x, y, pts, special = false) => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext("2d");
    const start = performance.now();
    const color = special ? "#ff8a18" : pts >= 40 ? "#ffe36e" : pts >= 25 ? "#ffcc44" : "#ff8844";
    const frame = (now) => {
      ...
      for (let index = 0; index < (special ? 18 : 8); index += 1) {
  ...
  const animateArrow = (pts, done, special = false) => {
    const canvas = canvasRef.current;
    ...
    const width = wrap.offsetWidth || 900;
    const height = 280;
    canvas.width = width;
    canvas.height = height;
    const wrapRect = wrap.getBoundingClientRect();
    const targetRect = targetRef.current?.getBoundingClientRect();
    const archerRect = document
      .querySelector(".ad-active-archer")
      ?.getBoundingClientRect();
    const startX = archerRect
      ? archerRect.left - wrapRect.left + archerRect.width * 0.72
      : 48;
    const startY = archerRect
      ? archerRect.top - wrapRect.top + archerRect.height * 0.55
      : height - 78;
    const targetX = targetRect
      ? targetRect.left - wrapRect.left + targetRect.width * 0.49
      : width * 0.86;
    const targetY = targetRect
      ? targetRect.top - wrapRect.top + targetRect.height * 0.35
      : height * 0.58;
    ...
    const ctx = canvas.getContext("2d");
    const frame = (now) => {
      const raw = Math.min((now - started) / 560, 1);
      const t = 1 - Math.pow(1 - raw, 3);
      ctx.clearRect(0, 0, width, height);
      const x = startX + (endX - startX) * t;
      const y = startY + (endY - startY) * t - Math.sin(raw * Math.PI) * 35;
        drawArrow(
        ctx,
        x,
        y,
      ...
      if (raw < 1) animationRef.current = requestAnimationFrame(frame);
        ...
        if (pts > 0 || special) impact(endX, endY, pts, special);
        setTimeout(
          () => {
            ctx.clearRect(0, 0, width, height);
            done();
  ...
  const chooseTile = (idx) => {
    ...
    const currentScore = scores[currentTurn];
    const pointsEarned = shot.special ? currentScore : shot.pts;
    setBusy(true);
    setRevealedIdx(idx);
    setChosenIdx(idx);
    setFloatingPoints({ value: pointsEarned, id: Date.now() });
    setResultText(
      shot.special
        ? "🟠 ORANGE SPECIAL — SCORE DOUBLED"
        : shot.pts > 0
    ...
    setResultCls(shot.special || shot.pts > 0 ? "hit" : shot.pts < 0 ? "miss" : "");
    setTimeout(
      () =>
        animateArrow(shot.special ? 1 : shot.pts, () => {
          setScores((previous) => {
            const next = {
              ...previous,
              [currentTurn]: shot.special
                ? previous[currentTurn] * 2
                : Math.max(0, previous[currentTurn] + shot.pts),
            };
            setSpecials((previousSpecials) => {
              const updated = { ...previousSpecials };
              Object.keys(updated).forEach((player) => {
                updated[player] = {
                  ...updated[player],
                  cooldown: Math.max(0, updated[player].cooldown - 1),
                };
              });
              if (shot.special) updated[currentTurn] = { unlocked: true, cooldown: 4 };
              else if (previous[currentTurn] <= 50 && next[currentTurn] > 50) {
                updated[currentTurn] = { unlocked: true, cooldown: 0 };
              }
              return updated;
            });
            if (next.student >= WIN_TARGET || next.teacher >= WIN_TARGET) {
              setScreen("win");
              onComplete?.(
                next[currentTurn],
                Math.round((next[currentTurn] / WIN_TARGET) * 100),
              );
            } else {
              const nextTurn =
                currentTurn === "student" ? "teacher" : "student";
              setCurrentTurn(nextTurn);
              setTimeout(() => newRound(nextTurn), 850);
            }
            return next;
          });
        }),
  ...
  const winnerName = picks[winner]
    ? `${COLORS[picks[winner]].name} Archer (${winner === "student" ? "Student" : "Teacher"})`
    : "";

  const renderSetupPanel = (player) => {
    const active = player === "student" ? setupStep === 0 : setupStep === 1;
    const chosen = player === "student" && setupStep === 1 ? picks.student : selectedColor;
    return (
      <section className={`setup-player-panel${active ? " active" : " locked"}`}>
        <div className="setup-player-heading">
          <span>{player === "student" ? "PLAYER 1" : "PLAYER 2"}</span>
          <strong>{player === "student" ? "Student" : "Teacher"}</strong>
          <small>{active ? "Choose your archer" : chosen ? `${COLORS[chosen].name} selected` : "Waiting for Player 1"}</small>
        </div>
        <div className="sprite-grid">
          {COLOR_KEYS.map((key) => {
            const taken = player === "teacher" && key === picks.student;
            return (
              <button
                key={key}
                className={`sprite-card${chosen === key ? " selected" : ""}${taken ? " taken" : ""}`}
                disabled={!active || taken}
                onClick={() => setSelectedColor(key)}
              >
                <ArcherSvg colorKey={key} facing="right" width="132" height="180" viewBox="0 0 72 110" />
                <div className="sprite-card-label">{COLORS[key].name}</div>
              </button>
            );
          })}
        </div>
  ...
  return (
    <div className="archer-duel-root">
      <style>{`
    ...
    `}</style>
      <style>{`
        #setupScreen{width:min(100%,1180px);padding:36px 20px 48px;margin:0 auto}
        .setup-panels{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:24px;align-items:stretch;margin:28px auto 24px}
        .setup-player-panel{padding:24px;border:1px solid var(--gold-dark);border-radius:18px;background:linear-gradient(150deg,#192f20,#101d16);box-shadow:0 18px 36px #0005}
        .setup-player-panel.active{border-color:var(--gold);box-shadow:0 0 0 2px #d8ad4533,0 18px 36px #0006}
        .setup-player-panel.locked{opacity:.65}
        .setup-player-heading{text-align:center;margin-bottom:18px}
        .setup-player-heading span{display:block;color:var(--gold);font:700 .75rem Georgia,serif;letter-spacing:.24em}
        .setup-player-heading strong{display:block;color:var(--gold-light);font:900 1.7rem Georgia,serif;margin:5px 0}
        .setup-player-heading small{color:var(--muted);font-size:.9rem;font-style:italic}
        .setup-player-panel .sprite-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin:0}
        .setup-player-panel .sprite-card{min-height:240px;padding:18px 10px 14px}
      ...
      {screen === "setup" && (
        <div id="setupScreen">
          <p className="setup-title">Choose Your Archers</p>
          <p className="setup-sub">First to 200 points wins the range.</p>
          <div className="setup-panels">
            {renderSetupPanel("student")}
            {renderSetupPanel("teacher")}
          </div>
          <button
      ...
      {screen === "game" && (
        <div id="gameScreen">
          <div className="scoreboard">
            <div
              className={`score-panel student${currentTurn === "student" ? " active-turn" : ""}`}
            >
              <div className="role">Student</div>
              <div className="player-name">
                {COLORS[picks.student].name} Archer
              </div>
              <div className="pts-value">{scores.student}</div>
              <div className="pts-label">points</div>
              <div className="progress-track">
                <div
                  className="progress-fill-s"
                  style={{
                    width: `${Math.min(100, (scores.student / WIN_TARGET) * 100)}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div
              className={`score-panel teacher${currentTurn === "teacher" ? " active-turn" : ""}`}
            >
              <div className="role">Teacher</div>
              <div className="player-name">
                {COLORS[picks.teacher].name} Archer
              </div>
              <div className="pts-value">{scores.teacher}</div>
              <div className="pts-label">points</div>
              <div className="progress-track">
                <div
                  className="progress-fill-t"
                  style={{
                    width: `${Math.min(100, (scores.teacher / WIN_TARGET) * 100)}%`,
                  }}
                />
              </div>
            </div>
          </div>
          <div className="arena-wrap" ref={arenaRef}>
            <ArenaScene />
            <TargetSvg shakeKey={shakeKey} targetRef={targetRef} />
            {currentTurn === "student" ? (
              <ArcherSvg
                colorKey={picks.student}
                facing="right"
                width="95"
                height="145"
                viewBox="0 0 72 110"
                className="ad-active-archer ad-student"
              />
            ) : (
              <ArcherSvg
                colorKey={picks.teacher}
                facing="right"
                width="95"
                height="145"
                viewBox="0 0 72 110"
                className="ad-active-archer ad-teacher"
              />
            )}
          ...
          {floatingPoints && (
            <div
              ...
              className={`floating-points ${floatingPoints.value >= 0 ? "positive" : "negative"}`}
          ...
          <div className="shot-turn-label">
            {COLORS[picks[currentTurn]].name} Archer's shot
          </div>
          <div className="tiles-section">
            <div className="tiles-label">— SELECT A TILE —</div>
            <div className="tiles-row">
              {currentShots.map((shot, index) => {
                const isRevealed = revealedIdx === index;
                const isChosen = chosenIdx === index;
                return (
                  <button
                    key={`${shot.label}-${index}`}
                    className={`tile tile-${index + 1}${shot.special ? " tile-special" : ""}${isRevealed ? " revealed" : ""}${isChosen ? " chosen" : ""}${busy && !isChosen ? " disabled" : ""}`}
                    disabled={busy}
                    onClick={() => chooseTile(index)}
                  >
                    <div className="tile-face">
                      <div className="tile-num">{index + 1}</div>
                      <div className="tile-q">TAP TO REVEAL</div>
                    </div>
                    <div className="tile-back">
                      <div className="rev-icon">{shot.icon}</div>
                      <div className="rev-label">{shot.label}</div>
                      <div
                        className={`rev-pts ${shot.pts > 0 ? "pos" : shot.pts < 0 ? "neg" : "zero"}`}
                      >
                        {shot.special ? "DOUBLE SCORE" : `${shot.pts > 0 ? "+" : ""}${shot.pts} pts`}
                      </div>
                    </div>
                  </button>
                );
      ...
      {screen === "win" && (
        <div id="winScreen">
          ...
          <div className="win-sub">
            First to reach {WIN_TARGET} points — magnificent!
          </div>
          <div className="final-scores">
            <div className="final-card">
              <div className="fc-role">STUDENT</div>
              <div className="fc-name">{COLORS[picks.student].name} Archer</div>
              <div className="fc-pts">{scores.student}</div>
            </div>
            <div className="final-card">
              <div className="fc-role">TEACHER</div>
              <div className="fc-name">{COLORS[picks.teacher].name} Archer</div>
              <div className="fc-pts">{scores.teacher}</div>
            </div>
          </div>
