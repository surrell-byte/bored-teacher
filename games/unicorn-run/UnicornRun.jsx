...
export default function UnicornRun({ onComplete }) {
  ...
  if (screen === "start") return (
    <div style={{
      ...
      background:"linear-gradient(135deg,#4a0072,#7b1fa2,#e91e63)",
      fontFamily:"'Segoe UI',sans-serif", color:"#fff", padding:24, textAlign:"center",
    }}>
      <div style={{ fontSize:"3.5rem", marginBottom:8 }}>🦄</div>
      <h1 style={{ fontSize:"2rem", margin:"0 0 4px", color:"#fce4ec" }}>🦄 Unicorn Run</h1>
      <p style={{ color:"#f8bbd0", marginBottom:20 }}>Vocabulary Adventure</p>
      <input value={playerName} onChange={e=>setPlayerName(e.target.value)}
        placeholder="Enter your magical name"
        style={{ padding:"12px 20px", borderRadius:12, border:"none", background:"rgba(255,255,255,0.15)", color:"#fff", fontSize:"1rem", marginBottom:24, width:240, textAlign:"center" }}
      />
  ...
  if (screen === "gameover") return (
    <div style={{
      ...
      <div style={{ display:"flex", gap:12 }}>
        <button onClick={() => startLevel(level)} style={{ padding:"14px 28px", borderRadius:999, border:"none", background:"#ec4899", color:"#fff", fontWeight:700, cursor:"pointer" }}>🔄 Try Again</button>
        <button onClick={() => setScreen("start")} style={{ padding:"14px 28px", borderRadius:999, border:"none", background:"#374151", color:"#fff", fontWeight:700, cursor:"pointer" }}>🏠 Menu</button>
      </div>
    </div>
  );
          ...
          {current.options.map(opt => {
            ...
            return (
              <button key={opt} onClick={() => answer(opt)} style={{
                ...
                borderColor: isCorrect?"#4ade80":isWrong?"#f87171":"rgba(255,255,255,0.2)",
        ...
        {answered && (
          <div style={{ textAlign:"center", marginTop:16 }}>
            <p style={{ fontWeight:700, color:selected===current.correct?"#4ade80":"#f87171", marginBottom:12 }}>
              {selected===current.correct ? "🌟 Magical! Correct!" : `❌ Answer: "${current.correct}"`}
            </p>
            {lives > 0 && (
              <button onClick={next} style={{ padding:"14px 32px", borderRadius:999, border:"none", background:"linear-gradient(135deg,#ec4899,#be185d)", color:"#fff", fontWeight:800, cursor:"pointer" }}>
