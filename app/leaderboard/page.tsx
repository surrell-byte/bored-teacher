...
export default function LeaderboardPage() {
  ...
  return (
    <div className="lb-page">
      ...
      {/* ── Game filter tabs ───────────────────────────────── */}
      <div className="lb-filter-tabs shell-card" style={{ padding: '16px 20px', marginBottom: 16, overflowX: 'auto', display: 'flex', gap: 8, flexWrap: 'nowrap' }}>
        <button
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                {['#', 'Player', activeGame === 'all' ? 'Total Score' : GAME_NAMES[activeGame], 'Avg %', 'Games'].map(h => (
                  <th key={h} style={{ padding: '14px 16px', textAlign: 'left', fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--muted)', fontWeight: 700 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
      ...
      {selected && (
        <div className="shell-card" style={{ padding: '24px', marginTop: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <h2 style={{ fontFamily: 'var(--font-display, Syne)', fontSize: '1.2rem', fontWeight: 800 }}>
