...
# apply-tile-battle-fx.sh
# Adds: particle bursts, animated result banner, confetti + premium end screen,
# glassmorphism styling, and sound-effect hooks to games/tile-battle/TileBattle.jsx

TARGET="games/tile-battle/TileBattle.jsx"
STAMP=$(date +%Y%m%d-%H%M%S)
BACKUP_DIR="backup/tile-battle-fx-${STAMP}"

if [ ! -f "$TARGET" ]; then
  echo "Error: $TARGET not found. Run this from the project root." >&2
  exit 1
fi

mkdir -p "$BACKUP_DIR/games/tile-battle"
cp "$TARGET" "$BACKUP_DIR/games/tile-battle/TileBattle.jsx.bak"
echo "Backed up existing file to $BACKUP_DIR"

mkdir -p public/assets/sounds/tile-battle

cat > "$TARGET" << 'TILEBATTLE_EOF'
...
echo "Updated $TARGET"
echo "Reminder: drop matching mp3s into public/assets/sounds/tile-battle/"
echo "  flip.mp3 heavy.mp3 attack.mp3 heal.mp3 recover.mp3 steal.mp3 explosion.mp3 win.mp3 click.mp3 music.mp3"
echo "Missing files fail silently, so the game still works without them."
